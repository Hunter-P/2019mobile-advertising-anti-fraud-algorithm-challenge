...
class FeatureEngineering:
    def __init__(self):
        ...
        del testA, testB
        self.data = train.append(test).reset_index(drop=True)
        del train, test
        gc.collect()
    ...
    def run(self):
        ...
        print(data.info())
        try:
            data.to_csv("/data1/pengjiaxin/round2/new_all_data.txt", index=False)
        except:
            data.to_csv("data/round1_all_data.txt", index=False)
        return

        self.data = self.shrink_feature(self.data, 90)
    ...
    def fill_null(data):
        ...
        object_col = [i for i in data.select_dtypes(object).columns if i not in ['sid', 'label']]
        data[object_col] = data[object_col].fillna(9999)
        return data
    ...
    def nginxtime_feature(data):
        ...
        :return:
        """
        data['nginxtime_time'] = pd.to_datetime(data['nginxtime'] * 1e+6) + timedelta(hours=8)
        data['nginxtime_day'] = data['time'].dt.day
        data['nginxtime_hour'] = data['time'].dt.hour
    ...
    def if_empty(data):
        ...
        for c in ['pkgname', 'adidmd5', 'openudidmd5', 'macmd5']:
            data[c+'_empty'] = (data[c]=='empty').astype('str')
        return data
    ...
    def combine_feature(data):
        ...
        data['pkgname_mediashowid'] = data['mediashowid'].astype('str') + data['pkgname'].astype('str')  # 19.17
        data['pkgname_make'] = data['make'].astype('str') + data['pkgname'].astype('str')  # 8.9
        data['pkgname_osv'] = data['osv'].astype('str') + data['pkgname'].astype('str')
        data['pkgname_model'] = data['model'].astype('str') + data['pkgname'].astype('str')
        data['pkgname_apptype'] = data['apptype'].astype('str') + data['pkgname'].astype('str')
        data['log_count_pkgname_mediashowid'] = np.log10(
            data.groupby(['pkgname_mediashowid'])['sid'].transform('count'))
        data['pkgname_adunitshowid'] = data['adunitshowid'].astype('str') + data['pkgname'].astype('str')  # 1.56
        data['adunitshowid_mediashowid'] = data['adunitshowid'].astype('str') + data['mediashowid'].astype(
            'str')  # 1.85
        return data
...
class ModelConstruction:
    ...
    def run(self, categorical_features_indices):
        ...
        for index, (train_index, test_index) in enumerate(skf.split(self.train_data, self.label)):
            ...
            model = catboost.CatBoostClassifier(iterations=7000, depth=8, cat_features=categorical_features_indices,
                                                early_stopping_rounds=500, verbose=50, l2_leaf_reg=1, task_type='GPU',
                                                learning_rate=0.1, eval_metric='F1', max_ctr_complexity=5)
            ...
            try:
                ...
                joblib.dump(cv_model, 'data/' + str(index) + '_cv5_model')
            except:
            ...
            if index == 0:
                ...
                try:
                    feature_importance.to_csv('/data1/pengjiaxin/round2/feature90_importance_cv5.csv', index=False)
                except:
